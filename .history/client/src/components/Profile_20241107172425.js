...
const Profile = () => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.data && Object.keys(response.data).length === 0) {
          setIsNewProfile(true); // No profile found, create new one
        } else {
          setProfile(response.data); // Set existing profile
        }
        setLoading(false);
      } catch (error) {
        console.error(error);
  ...
  // Enable edit mode with the current profile data
  const handleEdit = () => {
    setEditMode(true);
    setFormData({ 
  ...
  const handleSave = async (e) => {
    ...
    try {
      const url = isNewProfile ? '/api/profile' : '/api/profile'; // POST for new profile, PUT for existing
