...
import { login, register } from '../services/authService';  // Make sure these functions are implemented correctly
...
function LoginRegister({ setIsLoggedIn }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let data;
      if (isLogin) {
        data = await login(email, password);  // Login
      } else {
        data = await register(username, email, password, zodiacSign);  // Registration
      }
